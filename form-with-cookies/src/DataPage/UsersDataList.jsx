...
const UsersDataList = () => {
  ...
  useEffect(() => {
    document.cookie = `locationFromData=Data-page; path=/data; max-age=${ date }; secure`
  }, [])

  const removeItem = (item, cardNo) => {
    localStorage.setItem('personal-info', JSON.stringify(removeFromLocalStorage(localData, item)))
    sessionStorage.setItem('personal-info-card', JSON.stringify(removeFromLocalStorage(sessionData, cardNo)))
    ...
    return (
      <li key={ item.name } className='user-data__item'>
        <button
          ...
          onClick={ () => removeItem(item, sessionData[index]) }>
